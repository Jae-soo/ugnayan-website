...
export default function FarcasterManifestSigner({ 
  domain = 'promised-moving-017.app.ohara.ai',
  onSuccess,
  onError
        ...
        if (isInMiniAppContext && !isAlreadySigned) {
          try {
            ...
            if (error && typeof error === 'object') {
              type SdkErrors = {
                RejectedByUser?: new (...args: unknown[]) => Error
                InvalidDomain?: new (...args: unknown[]) => Error
                GenericError?: new (...args: unknown[]) => Error
              }
              const sdkErrors = (sdk as unknown as { errors?: SdkErrors }).errors ?? {}
              ...
              type ErrLike = { code?: string; errCode?: string; message?: string }
              const errObj = error as ErrLike
  ...
  }, [isAlreadySigned, isCheckingSignedStatus, domain, onSuccess, onError])
