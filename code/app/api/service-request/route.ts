...
export async function POST(request: NextRequest) {
  try {
    ...
    const { type, description, residentName, residentEmail, residentPhone, residentAddress, documentType, purpose, complaintType, additionalInfo } = body
    if (!type || !description || !residentName || !residentEmail) {
      return NextResponse.json({ error: 'Missing required fields' }, { status: 400 })
    }
    const serviceRequest = await createServiceRequest({ type, description, residentName, residentEmail, residentPhone, residentAddress, documentType, purpose, complaintType, additionalInfo })
